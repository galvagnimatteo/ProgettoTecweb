...
function getProfile(){
	...
	<div class="row">
		...
		<div class="col_75">
		<input type="text" placeholder=" Username" name="username_profile"  value="'.$row['username'].'"  required> 
		</div>
		</div>
		...
		<input type="text" placeholder=" Name" name="name_profile" value="'.$row['nome'].'"  required   > 
			</div>
		</div>
		...
		<div class="col_75">
		<input type="text" placeholder=" Surname" name="surname_profile"   value="'.$row['cognome'].'"   required>
		</div>
		</div>
		...
		<label for="email_profile"><span xml:lang="en">Email</span></label>
			...
			<div class="col_75">
		<input type="email" placeholder=" Email" name="email_profile"  value="'.$row['email'].'"   >
		</div>
		</div>
		...
		<div class="col_75">
		<input type="password" placeholder=" Password" name="password_profile"   value="'.$row['password'].'"   >
		</div>
		</div>
		...
		<label for="pass_profile_confirm"><span xml:lang="en">Password Confirmation</span></label>
			...
			<div class="col_75">
		<input type="password" placeholder=" Repeat Password" name="pass_profile_confirm"  value="'.$row['password'].'"   required>
	</div>
		</div>
...
function changeProfile(){
	...
	if(isset($_POST['name_profile'])  &&isset($_POST['password_profile'])
	&&isset($_POST['email_profile']) &&isset($_POST['surname_profile'])  && isset($_POST['password_profile']) && isset($_SESSION['a']) ){
	
	
	
		$db = SingletonDB::getInstance();
		
		$query = "UPDATE utenti SET nome=?, cognome=?, password=?,email=? WHERE username=?)";
		if($preparedQuery = $db->getConnection()->prepare($query))
		{
		$preparedQuery->bind_param('sssss',$name,$surname,$password,$email,$username);
